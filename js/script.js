...
$(function () {
    ...
    $(".submit_btn").click(function () {
        ...
        if (name == "") {
            $("#name_error").show();
            $("input#name").css({ border: "1px solid #ffd40a" });
            return false;
        }
        ...
        if (email == '') {
            $("input#email").css({ border: "1px solid #ffd40a" }).before('<div class="error_contact" id="email_error">Please enter your email address</div>');
            hasError = true;
        }

        else if (!emailReg.test(email)) {
            $("input#email").css({ border: "1px solid #ffd40a" }).before('<div class="error_contact" id="email_error">Enter a valid email address</div>');
            hasError = true;
        }
        ...
        if (comment == "") {
            $("#comment_error").show();
            $("textarea#comment").css({ border: "1px solid #ffd40a" });
            return false;
        }
...
$(document).ready(function () {
    ...
    $("#actions_phone a.love").click(function () {
        $("#skills_phone, #backend_phone").hide('fast');
        $('#love_phone').slideDown('slow');
    });
    $("#actions_phone a.skills").click(function () {
        $("#love_phone, #backend_phone").hide('fast');
        $('#skills_phone').slideDown('slow');
    });
    $("#actions_phone a.backend").click(function () {
        $("#love_phone, #skills_phone").hide('fast');
        $('#backend_phone').slideDown('slow');
    });
